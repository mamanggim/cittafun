...
(() => {
  ...
  function setInitialSidebarState() {
    if (window.innerWidth <= 900) {
      sidebar?.classList.remove("open"); // mobile: tertutup
    } else {
      sidebar?.classList.add("open"); // desktop: selalu terbuka
    }
  }
      ...
      try {
        if (firebase.firestore) {
          ...
          if (doc.exists) {
            ...
            if (rupEl && data.points != null)
              rupEl.textContent = `Rp${(data.points / 10).toLocaleString("id-ID")}`;
